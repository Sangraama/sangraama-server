...
/*public void loadTiles(List<Layer> layers){
	    ...
	    for(int i=0;i<layers.size();i++){
	    	switch(layers.get(i).getName()){
	    	case "Background": 
	    		backgroundTiles=layers.get(i).getData().getTiles();
	    		for(int k=0;k<backgroundTiles.size();k++){
	    		if(backgroundTiles.get(k).getGid()>0){
	    			backgroundLayerTiles.add(backgroundTiles.get(k));
	    		}
	    		}
	    		break;
	    	case "Foreground": 
	    		foregroundTiles=layers.get(i).getData().getTiles();
	    		for(int k=0;k<foregroundTiles.size();k++){
		    		if(foregroundTiles.get(k).getGid()>0){
		    			foregroundLayerTiles.add(foregroundTiles.get(k));
		    		}
		    		}
	    		break;
	    	case "Top": 
	    		topTiles=layers.get(i).getData().getTiles();
	    		for(int k=0;k<topTiles.size();k++){
		    		if(topTiles.get(k).getGid()>0){
		    			topLayerTiles.add(topTiles.get(k));
		    		}
		    		}
	    		break;
	    	}
