...
public class PhysicsAPI {
    ...
    public void applyPhysics(StaticObject staticObject) {
        ...
        if (staticObject.getType().equals("polygon")) {
            ...
            for (int i = 0; i < vertices.length; i++) {
                vertices[i] = new Vec2();
                vertices[i].set(staticObject.getCoordinates().get(i).getX(), staticObject
                        .getCoordinates().get(i).getY());
            }
    ...
    public void applyPhysics(List<StaticObject> staticObjects, World world) {
        float xLimit = SangraamaMap.INSTANCE.getOriginX() + SangraamaMap.INSTANCE.getMapWidth(); // The
                                                                                                 // limit
                                                                                                 // in
                                                                                                 // X-axis
                                                                                                 // of
                                                                                                 ...
                                                                                                 // the
                                                                                                 // server.
        float yLimit = SangraamaMap.INSTANCE.getOriginY() + SangraamaMap.INSTANCE.getMapHeight(); // The
                                                                                                  // limit
                                                                                                  // in
                                                                                                  // Y-axis
                                                                                                  // of
        ...
        for (int i = 0; i < staticObjects.size(); i++) { // for each static object
            int count = 0;
            for (int k = 0; k < staticObjects.get(i).getCoordinates().size(); k++) { // for each
                                                                                     // coordinate
                                                                                     // of the
                                                                                     // object
                if (staticObjects.get(i).getX() < xLimit && staticObjects.get(i).getY() < yLimit) { // if
                                                                                                    // the
                                                                                                    // x
                                                                                                    // and
                                                                                                    // y
                    ...
                    Body newStaticObjectBody = world.createBody(this.getBodyDef()); // add the
                                                                                    // static object
                                                                                    // to the game
                                                                                    // world.
    ...
    public void applyPhysics(Bullet bullet, World world) {
        ...
        /*
         * Body newPlayerBody = world.createBody(player.getBodyDef());
         * newPlayerBody.createFixture(player.getFixtureDef()); player.setBody(newPlayerBody);
         */
    }
