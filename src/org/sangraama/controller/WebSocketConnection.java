...
public class WebSocketConnection extends MessageInbound {
	...
	protected void onTextMessage(CharBuffer charBuffer) throws IOException {
		...
		if (this.player != null) {
			...
			case 1: // setting user event request
				...
				this.player.setAngle(p.getV_a());
				System.out.println(TAG + " set user events " + p.getV_x()
						+ " : " + p.getV_y());
				break;
			case 2: // requesting for interesting area
				this.player.setInterestingIn(p.getX(), p.getY());
				System.out.println(TAG + "player interesting in x:" + p.getX()
						+ " y:" + p.getY());
				break;
			...
			if (p.getType().equals("1")) { // create new player & set the
											// connection
				this.player = new Player(p.getUserID(), p.getX(), p.getY(),
						this);
	...
	public void sendUpdate(ArrayList<PlayerDelta> deltaList) {
		...
		try {
			getWsOutbound().writeTextMessage(
					CharBuffer.wrap(gson.toJson(deltaList)));
		} catch (IOException e) {
	...
	/**
	 * Send new connection details as a list. Because updates are send as a
	 * list, sending new single connection details can't recognize by client
	 * side.
	 * 
	 * @param transferReq
